...
const CenterModal = ({ center, onClose }) => {
  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.48)",
    ...
    <div>
      ...
      <div style={{ ...cardStyle, marginBottom:20 }}>
        ...
        <ResponsiveContainer width="100%" height={260}>
          <AreaChart data={MONTHLY_TREND} margin={{ top:5, right:20, left:20, bottom:5 }}>
            <defs>
              {[["awsG","#ff9900"],["azG","#0078d4"],["gcpG","#4285f4"]].map(([id,col])=>(
            ...
            <Area type="monotone" dataKey="AWS"    stroke="#ff9900" strokeWidth={2} fill="url(#awsG)" dot={false}/>
            <Area type="monotone" dataKey="Azure"  stroke="#0078d4" strokeWidth={2} fill="url(#azG)"  dot={false}/>
            <Area type="monotone" dataKey="GCP"    stroke="#4285f4" strokeWidth={2} fill="url(#gcpG)" dot={false}/>
            <Area type="monotone" dataKey="budget" stroke={C.danger} strokeWidth={1.5}
              strokeDasharray="6 3" fill="none" name="Budget Ceiling"/>
          </AreaChart>
        </ResponsiveContainer>
      </div>
...
export default function FinOpsDashboard() {
  ...
  const filtered = BILLING_CENTERS
